...
public class BallWorld extends JPanel implements Serializable {
    ...
    public BallWorld(int width, int height){
        ...
        for(int i = 0; i <= size; i++){              //yukarıda oluşturulan rastgele sayıya kadar döngü
            ...
            /*else*/ if(size2 == 2){
                //generate Ball
                generate.add(new Ball(x+randomX,y+randomY,radius,speed+random_speed,angleInDegree+random_angle,Color.blue));  // Ball üretir ve listeye ekler...
                ballcount++;
            }/*
            else if(size2 == 3){
                //generate Arc
    ...
    public void createReceivedBall (SerializableBall ballInfo){
        generate.add( new Ball(20, ballInfo.y, 20, 10, ballInfo.moveAngle, Color.BLUE)) ;//40 is double of radius
        generate.get(-1).speedX = -generate.get(-1).speedX  ;//to get correct direction of movement
        gameStart();
    }
